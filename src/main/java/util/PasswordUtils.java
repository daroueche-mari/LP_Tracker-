package util;
// Importations des classes nécessaires pour le fonctionnement de la classe utilitaire de gestion des mots de passe (ex: pour générer des sels, hasher les mots de passe, etc.)
...
public class PasswordUtils { // Classe utilitaire pour la gestion des mots de passe (ex: pour générer des sels, hasher les mots de passe, etc.)
    ...
    // Méthode pour générer un sel aléatoire pour le hashage des mots de passe (ex: lors de l'inscription d'un nouvel utilisateur)
    ...
    // Méthode pour hasher un mot de passe en utilisant SHA-256 avec un sel et un poivre (ex: lors de l'inscription ou de la vérification du mot de passe lors de la connexion)
